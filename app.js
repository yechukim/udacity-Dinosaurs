...
function getRandom(dino) {
  const factsArray = [
    ...
    dino.where,
    dino.fact,
    dino.compareWeight(dino),
    dino.compareHeight(dino),
    dino.compareDiet(dino),
  ];
...
Dino.prototype.compareDiet = function (dino) {
  if (humanData().diet === dino.diet)
    return `${dino.species} has a same taste with you!`;
  return `${dino.species} does not have a same taste with you!`;
};

Dino.prototype.compareWeight = function (dino) {
  if (!humanData().weight)
    return `${dino.species} is ${dino.weight} in lbs, how about yours?`;
  if (humanData().weight < dino.weight)
    return `${dino.species} is heavier than you`;
